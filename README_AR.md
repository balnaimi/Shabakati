...
- 🔎 بحث عالمي في الأجهزة والشبكات
- 📈 لوحة التشغيل (uptime) مع سجل لكل جهاز
- 🔔 تنبيهات المسح التلقائي (داخل التطبيق، متصفح اختياري، webhook)
- 📤 تصدير/استيراد JSON ونسخ احتياطي مجدول على السيرفر
- 📱 دعم تثبيت PWA
- 🏭 تعرف على المصنّع (OUI) وتصنيف الأجهزة أثناء المسح
...
- **الإعدادات (Settings)** - الإصدار، سجل التغييرات، التنبيهات، تصدير/استيراد، webhook (للمسؤول)
...
لا تستخدم `docker compose down -v` إلا إذا أردت مسح volume قاعدة البيانات.

تحقق من الإصدار من التطبيق: **الإعدادات** (قائمة المستخدم) أو `GET /api/health` → `{ "version": "…" }`.

---

## ⚙️ التشغيل (Production)

| الميزة | الإعداد |
|--------|---------|
| **توقيع JWT** | مطلوب في الإنتاج: `JWT_SECRET` في `.env` (`openssl rand -base64 48`) |
| **Webhook** | مسؤول → **الإعدادات** → رابط Webhook، أو `WEBHOOK_URL` في `.env` (البيئة تتقدّم) |
| **نسخ احتياطي مجدول** | `BACKUP_INTERVAL_HOURS=24` (0 = معطّل)، `BACKUP_RETENTION_COUNT=7`، و`BACKUP_DIR` اختياري |
| **نسخ يدوي** | مسؤول → **الإعدادات** → تصدير / «نسخ احتياطي الآن» |
| **Uptime** | تلقائي مع المسح؛ صفحة **التشغيل** في الشريط |
| **تنبيهات المسح** | تنبيهات داخل التطبيق؛ تنبيهات المتصفح اختيارية من **الإعدادات** |
| **Reverse proxy** | `TRUST_PROXY=1` خلف nginx/Traefik |

مثال حمولة Webhook (`event`: `auto_scan_alert`):
